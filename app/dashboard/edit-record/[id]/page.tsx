...
interface Record {
  ...
  endTime?: string | null
  expenseFuel?: number | null
  expenseMaintenance?: number | null
  expenseFood?: number | null
  expenseWash?: number | null
  expenseToll?: number | null
  expenseParking?: number | null
...
export default function EditRecordPage() {
    ...
    async function loadRecord() {
      try {
        ...
        setFormData({
          ...
          kmPerLiter: data.kmPerLiter ? data.kmPerLiter.toString() : '',
          tripsCount: data.tripsCount ? data.tripsCount.toString() : '',
          hoursWorked: data.hoursWorked ? data.hoursWorked.toString() : '',
          startTime: data.startTime || '',
          endTime: data.endTime || '',
          expenseFuel: data.expenseFuel ? data.expenseFuel.toString() : '',
          expenseMaintenance: data.expenseMaintenance
            ? data.expenseMaintenance.toString()
            : '',
          expenseFood: data.expenseFood ? data.expenseFood.toString() : '',
          expenseWash: data.expenseWash ? data.expenseWash.toString() : '',
          expenseToll: data.expenseToll ? data.expenseToll.toString() : '',
          expenseParking: data.expenseParking
            ? data.expenseParking.toString()
            : '',
          expenseOther: data.expenseOther ? data.expenseOther.toString() : '',
          notes: data.notes || '',
        })
      } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.kmPerLiter && parseFloat(formData.kmPerLiter) < 0) {
        setError('Média KM/L não pode ser negativa')
        setSaving(false)
        return
      }
      ...
      setError(err.message || 'Erro ao atualizar registro. Tente novamente.')
      setSaving(false)
    }
  }
      ...
      <DashboardLayout userName={userName}>
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            <p className="text-gray-600 mb-4">Registro não encontrado</p>
            <Link href="/dashboard">
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <p className="text-xs text-gray-500 -mt-2">
                Informe a média de km/litro do seu veículo para calcular o consumo de combustível do dia
              </p>

              {/* Horas Trabalhadas e Horário */}
